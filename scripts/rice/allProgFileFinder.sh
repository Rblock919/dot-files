...
then
  ...
  then
    while read -r line; do
      echo "$line" \?\?
    done <<< $(find "/hdd/Programming/" -type d -not -path "/hdd/Programming/**/node_modules/*" -iname *"${QUERY#\?}"* 2>&1 | grep -v 'Permission denied\|Input/output error')
  else
    find "/hdd/Programming/" -type d -not -path "/hdd/Programming/**/node_modules/*" -iname *"${QUERY#!}"* 2>&1 | grep -v 'Permission denied\|Input/output error'
  fi
else
  ls /hdd/Programming/
